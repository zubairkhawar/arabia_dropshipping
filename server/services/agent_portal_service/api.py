...
# Auto-offline on WebSocket disconnect was removed: mobile browsers aggressively
# close background WebSockets, causing agents to "go offline" and chats to be
# released to the bot within ~1 min of inactivity. Agent status now changes only
# via explicit toggle (POST /api/routing/agents/{id}/status). Attendance sessions
# still close on idle via the Redis TTL (attendance_idle_ttl_seconds) so working-
# hours tracking is unaffected.
